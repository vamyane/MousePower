...
- **鼠标穿透（默认开启）**：浮窗是纯显示层，鼠标点击直接穿透到下层软件，**不遮挡任何操作**；位置用设置里的「屏幕位置」四角预设调整
- **背景 / 内容独立透明度**：背景卡片（5%–100%）与文字图标（20%–100%）分别可调，可做出「只留文字、背景全透」的悬浮效果（基于逐像素 alpha 分层窗口实现）
- **系统托盘**：百分比数字 / 电池条两种图标，悬停显示设备与电量详情，右键可一键切换鼠标穿透
- **置顶浮窗**：屏幕右下角常驻电量卡片，自动跟随鼠标所在显示器，支持位置记忆
- **深度自定义**：深/浅/跟随系统主题 · 8 色 Fluent 强调色 · 三档尺寸 · 位置锁定
...
2. 浮窗默认开启**鼠标穿透**（点击不遮挡下层软件）；要拖动它请先在设置里关闭穿透，或直接用设置里的「屏幕位置」四角预设
3. **右键托盘图标**：立即刷新 / 显示隐藏浮窗 / **鼠标穿透开关** / 设置 / 退出
4. **设置窗口**：背景与内容透明度、鼠标穿透、锁定位置、屏幕位置、主题、强调色、尺寸、通知阈值、开机自启、刷新间隔
